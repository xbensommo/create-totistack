...
export const generateActionFiles = (actionsDir, collections, authCollections, roles) => {
  try {
    for (const col of collections) {
      const camelCol = toCamel(col.name);
      const pascalCol = capitalize(camelCol);
      const isAuth = authCollections.includes(col.name);
...
/**
 * Generates a set of Firestore actions scoped to the \`${col.name}\` collection.
 *
 * @param {Object} state - The reactive state from a Pinia store instance.
 * @returns {Object} A set of methods to interact with the \`${col.name}\` Firestore collection.
 */
export function use${pascalCol}Actions(state) {
  ...
  return {
    /**
     * Fetches the initial page of \`${col.name}\` documents based on current filters and sorting.
    ...
    /**
     * Fetches the next page of \`${col.name}\` documents (for load more functionality).
     * @returns {Promise<void>}
     */
    fetchNextPage${pascalCol}: actions.fetchNextPage,

    /**
     * Applies filters to the \`${col.name}\` collection and re-fetches the initial page.
    ...
    /**
     * Changes the sorting of the \`${col.name}\` collection and re-fetches the initial page.
    ...
    /**
     * Adds a new document to the \`${col.name}\` collection.
     * @param {Object} data - The data for the new \`${col.name}\` document.
    ...
    /**
     * Updates an existing document in the \`${col.name}\` collection.
    ...
    /**
     * get an existing document in the \`${col.name}\` collection.
     * @param {string} id - The ID of the document to update.
     * @param {Object} data - The partial data to update the document with.
     * @returns {Object}
     */
    ...
    /**
     * Performs a search on the \`${col.name}\` collection.
    ...
    /**
     * Clears any active search results for the \`${col.name}\` collection.
     * @returns {void}
     */
    clear${pascalCol}Search: actions.clearSearch,

    /**
     * Deletes a document from the \`${col.name}\` collection.
    ...
    ${isAuth ? `
    /**
     * Assigns roles to a user within the \`${col.name}\` context.
    ...
    /**
     * Revokes roles from a user within the \`${col.name}\` context.
     * Applicable primarily to authentication-related collections (e.g., 'users').
     * @param {string} userId - The ID of the user.
