...
import { useAuthStore } from '@/stores/authStore'; // Assuming authStore exists if auth is enabled
...
export async function ${guardName}Guard(to, from, next) {
    ...
    if (to.meta.requiresRole && (!authStore.user || !authStore.user.roles.includes(to.meta.requiresRole))) {
    ...
    // if (to.meta.requiresPermission && (!authStore.user || !authStore.user.permissions.includes(to.meta.requiresPermission))) {
