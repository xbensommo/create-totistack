...
export async function setupTailwind() {
  ...
  if (await fs.pathExists(appVuePath)) {
    ...
    if (scriptSetupRegex.test(appVueContent) && !appVueContent.includes(`import PageSpinner from '@/components/PageSpinner.vue';`)) {
    ...
    if (templateRegex.test(appVueContent) && !appVueContent.includes(`<PageSpinner />\n<ViewRouter />`)) {
      appVueContent = appVueContent.replace(
        '</template>',
        ` <ViewRouter />\n <PageSpinner />\n</template>`
      );
